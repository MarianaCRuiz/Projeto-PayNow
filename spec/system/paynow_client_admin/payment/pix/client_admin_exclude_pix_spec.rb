...
describe 'client_admin deactivate credit card option' do
  ...
  let(:pay4) { PaymentOption.create!(name: 'PIX_2', fee: 1.7, max_money_fee: 22, payment_type: 2) }
  ...
  it 'client_admin deactivate credit card succesfully' do
    ...
    pix2 = PixRegisterOption.create!(company: company, payment_option: pay4, pix_key: 'HSBCU6874bGABPEH62B2',
                                     bank_code: bank)
    DomainRecord.find_by(email_client_admin: user_admin.email).update!(company: company)
    Charge.create!(client_token: final_client.token, client_name: final_client.name,
                   client_cpf: final_client.cpf, company_token: company.token,
                   product_token: product.token, payment_method: pay_pix1.name,
                   due_deadline: '20/08/2021', company: company, final_client: final_client,
                   status_charge: status_charge, product: product,
                   payment_option: pay_pix1, price: product.price, charge_price: 45,
