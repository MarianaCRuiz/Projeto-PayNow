...
describe Company do
  context 'validation' do
    ...
    it ' must be uniq' do
      Company.create!(corporate_name: 'test SA', cnpj: '11.222.333/0001-45',
                      state: 'São Paulo', city: 'Campinas', district: 'Inova',
                      street: 'rua 1', number: '12', address_complement: '',
                      billing_email: 'persontest@test.com')
      company2 = Company.new(corporate_name: 'test SA', cnpj: '11.222.333/0001-45',
      ...
      expect(company2.errors[:corporate_name]).to include('já está em uso')
      expect(company2.errors[:cnpj]).to include('já está em uso')
      expect(company2.errors[:billing_email]).to include('já está em uso')
    end
    it ' format' do
      company2 = Company.new(corporate_name: 'test SA', cnpj: '43i4943i9034i43',
                             state: 'São Paulo', city: 'Campinas', district: 'Campos',
                             street: 'rua 2', number: '13', address_complement: '',
                             billing_email: 'persontest@test.com')
      ...
      expect(company2.errors[:cnpj]).to include('formato XX.XXX.XXX/XXXX-XX')
    end
  end
end
