...
describe Charge do
  ...
  let(:pay_creditcard_1) {PaymentOption.create!(name: 'Cartão de crédito', fee: 1.9, max_money_fee: 20, payment_type: 1)}
  ...
  context 'validation' do
    ...
    it 'presence additional data for boleto' do
      PaymentCompany.create(company: company, payment_option: pay_boleto_1)
      charge1 = Charge.new(payment_option: pay_boleto_1)
      charge1.valid?
      expect(charge1.errors[:client_name]).to include('não pode ficar em branco') 
      expect(charge1.errors[:client_cpf]).to include('não pode ficar em branco') 
      expect(charge1.errors[:company_token]).to include('não pode ficar em branco') 
    ...
    it 'presence additional data for credit card' do
      ...
      charge1 = Charge.new(payment_option: pay_creditcard_1)
      charge1.valid?
      expect(charge1.errors[:client_name]).to include('não pode ficar em branco') 
      expect(charge1.errors[:client_cpf]).to include('não pode ficar em branco') 
      expect(charge1.errors[:company_token]).to include('não pode ficar em branco') 
