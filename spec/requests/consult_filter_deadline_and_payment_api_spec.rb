...
describe 'consult charges api' do
  ...
  context 'filter payment method and due deadline' do
    it 'filter due deadline specific' do
      ...
      get '/api/v1/consult_charges',
          params: { consult: { payment_method: pay_boleto1.name, due_deadline: '20/08/2021' },
    ...
    it 'filter due deadline max' do
      ...
      get '/api/v1/consult_charges',
          params: { consult: { payment_method: pay_boleto1.name, due_deadline_max: '20/09/2021' },
                    company_token: company.token }
    ...
    it 'filter due deadline min' do
      ...
      get '/api/v1/consult_charges',
          params: { consult: { payment_method: pay_creditcard1.name, due_deadline_min: '20/09/2021' },
                    company_token: company.token }
      ...
      parsed_body = JSON.parse(response.body)
      expect(parsed_body.count).to eq(1)
      expect(parsed_body[0]['product_name']).to eq('Produto 2')
      expect(parsed_body[0]['price']).to eq('60.0')
      expect(parsed_body[0]['discount']).to eq('3.0')
      expect(parsed_body[0]['charge_price']).to eq('57.0')
      expect(parsed_body[0]['client_name']).to eq('Cliente final 2')
      expect(parsed_body[0]['client_cpf']).to eq('11133355599')
      expect(parsed_body[0]['payment_method']).to eq('Cartão de Crédito MasterChef')
    end
    it 'filter due deadline min and max' do
      ...
      get '/api/v1/consult_charges',
          params: {
            consult: { payment_method: pay_creditcard1.name, due_deadline_min: '20/09/2021',
                       due_deadline_max: '20/09/2024' }, company_token: company.token
          }
  ...
  context 'failure' do
    it 'method and due deadline invalid' do
      CompanyClient.create!(final_client: final_client, company: company)
      CompanyClient.create!(final_client: final_client2, company: company)
      product
      product2
      ...
      get '/api/v1/consult_charges',
          params: { consult: { payment_method: 'Caqui', due_deadline: '20/05/2050' }, company_token: company.token }
